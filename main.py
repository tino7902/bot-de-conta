...
async def get_num2(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        await update.message.reply_text("No te entendí. Por favor asegurate el que el numero que me mandaste no tenga otras letras o puntos")
        context.user_data['num1'] = int(update.message.text)
    await update.message.reply_text("¿Esta cuenta tiene IVA del 5% o 10%?\n(1)- 10% de IVA\n(2)- 5% de IVA")
    return OPERATION
